...
export const decodeSessionCookie = (encoded: string): { key: string; value: string } | null => {
...
export const parseCookies = (setCookieHeader: string): Record<string, string> => {
  ...
  console.log('Split cookie strings:', cookieStrings);
...
export const getSessionKeyFromCookies = (cookies: Record<string, string>): string | undefined => {
  return Object.keys(cookies).find(key => key.startsWith('wp_woocommerce_session_'));
}; 